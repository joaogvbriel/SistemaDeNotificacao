...
@Component
public class NotificationsHighPriority implements KafkaConsumerService{
    ...
    @KafkaListener(topics = "notificacoes.request.topic.v1.high", groupId = "grupo-prioridade-high",containerFactory = "kafkaListenerContainerFactory")
    public void listenHigh(@Payload NotificationsDTO notifications,
                           @Header(KafkaHeaders.RECEIVED_TOPIC) String topic,
                           @Header(KafkaHeaders.OFFSET) Long offset,
                           Acknowledgment acknowledgment){
        logs.log("Mensagem recebida na prioridade [HIGH].","INFO");
    ...
    public void process() {
        ...
        if (notifications != null) {
            logs.log("Processando mensagem na prioridade [HIGH].","INFO");
            try {
                ...
                logs.log("Mensagem processada com sucesso na prioridade [HIGH].","INFO");
            } catch (Exception e) {
                ...
                logs.logWithMap("Erro ao processar mensagem na prioridade [HIGH].","ERROR", context);
            }
        }
    }
